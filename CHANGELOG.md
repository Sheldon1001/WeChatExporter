...
### Fixed
- **同名会话不再互相覆盖**（macOS）：同一批导出共用一个时间戳，两个重名的「老王」或两个同名群会拼出完全一样的文件夹名，写进同一个夹后 chat.json / chat.txt 被后一个覆盖
  - 夹名改由 `ExportNaming.uniqueFolderNames` 批量算，只给真的撞名的那些追加 wxid 尾段（`老王_123456_<时间戳>/`），不撞名的保持 `老王_<时间戳>/` 不变
- **跨卷引用的视频不再被拷两份**（macOS）：分卷时各卷各自做引用去重，同一个视频被第 1 卷和第 3 卷引用时会两次走到外链落盘，第二次撞名改成 `clip_2.mp4` 又拷了一遍——几百 MB 的视频每多一卷就多占一份，且第 2 卷的播放器指向的是副本
  - `ExternalMediaSink` 增加按源文件路径的缓存，同一个文件重复请求直接拿回首次的相对路径。表情画廊跨页分组时同样受益
- **`.flv` / `.wmv` 视频渲染成播放器而非附件链接**（macOS）：网页导出私藏了一份视频扩展名表，比「分类导出」那份少了这两个，于是同一个文件被渲染成附件链接、却又被归进 `media/视频/`。现在统一用 `MediaOrganizer.category`
...
### Changed
- **每次导出的每个会话各自成夹**（macOS）：产物统一落到 `<联系人>_<时间戳>/`，四种导出方式一致
  - 此前网页导出把多卷 HTML 与 `<名称>_<时间戳>_media/` 直接摊在导出根目录里，连导几个会话之后根目录全是同前缀的文件，分不清谁属于谁
  - 分类导出等模式此前用的是不带时间戳的 `<联系人>/`，同一个会话导第二次会和上一次的产物混在一起，现在各自成卷
  - 文件夹名已带时间戳，网页文件名不再重复带：`张三_1.html`、`张三_2.html`，单卷时就是 `张三.html`
  - 同一批导出的所有会话共用一个时间戳，文件夹在文件管理器里排在一起
- **网页导出的外链媒体按类型分目录**：`media/` 下再分 `视频 / 图片 / 语音 / 其他`，分类表与「分类导出」共用一份（`MediaOrganizer.category`）

### Fixed
- **表情包画廊不再是一个几百 MB 的巨型网页**（实测一份真实数据 427.9 MB → 每页十几 KB）
  - 此前每张表情都 base64 内嵌进同一个 HTML，浏览器打开就卡死。现在画廊也进自己的 `全部表情包_<时间戳>/`，表情作为文件外链到 `media/图片/`，浏览器按需加载
  - 每页 500 张自动分页，页眉带上下页导航；分组跨页时标题会在新页重复，并显示本页张数
